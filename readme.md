# Учебно-исследовательская тема: создать теорию и программное обеспечение для изучения ориентаций линий сгиба при многократном сворачивании прямоугольного листа бумаги.

Сайт: https://paperteam1.herokuapp.com/constructor
...
- дан результат сгибов, восстановить последовательность осей сгибов и вообще, возможность получения такой картины сгибов; написать программу, выполняющую эти операции
- придумать и написать серию теорем о том, как могу и как не могут выглядеть ориентации линий сгиба
- создать игровой сайт, на котором будут генерироваться задачи вида (там же должен быть удобный интерфейс для ввода решений и проверки ответов)

> “Робот осуществляет сборку конструкции сгибами листа. На рисунке представлено задание роботу: сплошная линия показывает сгибы в одну сторону, пунктиров - в другую.

Управление осуществляется указанием осей сгибов (ось задается одной буквой, после каждого сгиба некоторые оси совмещаются друг с другом, в таких случаях неважно, какую ось указывать).

Укажите последовательность осей сгибов так, чтобы конструкция приобрела максимально свернутый вид. Направление сгиба определяет сам робот, однако, если программа сборки не может быть реализована, он прекращает свою работу.”

Комментарий к задаче

**Ответ: BEHC(A)D(F)G(I)**

> Подчеркнутые пары можно переставлять, а буквой в скобках заменять букву, стоящую перед ней. Всего 16 правильных строк.
...
## Примеры использования api алгоритма (python)
...
3. Создадим структуру папок и начнем модульную разработку применяя `Flux` архитектуру в нашем приложении
