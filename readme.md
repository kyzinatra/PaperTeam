...
![Рис.1. Пояснение терминов развертка, ось, сегмент.](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/how_it_should_work.drawio.svg)
...
![Рис. 2. Пример задачи.](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/trgt.png)
...
![Рис. 3. Представление развертки на начальной стадии. ](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/1.png)
...
![Рис. 4. Вид представления после первого сгиба.](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/2.png)
...
![Рис. 5. Вид представления после второго сгиба.](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/3.png)
...
![Рис. 6. Пример для пояснения алгоритма поиска всех путей.](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/Untitled.png)
...
![Рис. 7. Дерево путей для рис. 6.](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/Untitled%201.png)
...
## Оптимизация алгоритма по времени // не добавлять в отчет

Проделаем все тоже самое для развертки, данной в задании (рис. 9).

![Рис. 9. Развертка из задания.](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/Untitled%202.png)

Рис. 9. Развертка из задания.

На рис. отображено дерево путей. Очевидно, что оно хранит много лишних данных. К примеру если взять путь EHBDG и EHBDI, то понятно, что по факту они отображают один и тот же путь, поскольку оси I и G наложены друг на друга. А все действия, выполняемые алгоритмом для сворачивания по пути EHBDI уже выполнялись и просто отнимают время работы алгоритма.

Предпримем небольшую оптимизацию.

Для этого создадим список **doublesPaths.** 

После каждого сворачивания развертки будем сравнивать её вид с существующими в дереве. И если такая развертка уже есть, будем добавлять в doublesPaths пару путей: текущий путь и путь найденной копии в дереве.

Минусом алгоритма является то, что теперь нужно хранить не только площади разверток, а сами развертки.

![Рис. 10. Дерево путей для развертки из задания.](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/Screenshot_2022-05-15_122011.png)

Рис. 10. Дерево путей для развертки из задания.

![Рис. 11. Дерево путей после работы оптимизированного алгоритма. ](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/Untitled%203.png)

Рис. 11. Дерево путей после работы оптимизированного алгоритма. 

На рис. 11 представлено дерево путей после того, как новый алгоритм отработает.

Однако, получить решения с этого дерева стало сложней, поскольку не все листья теперь представляют собой конечные пути.

Самым простой способ получить ответ - это по полученному списку doublesPaths скопировать из всех копирующих путей необходимые части дерева. Тогда оно снова примет вид как на рис.  10.

И хотя мы получили такую же картину, да ещё и с хранением дополнительных данных, алгоритм в этом случае работает быстрее, поскольку не приходится проделывать довольно тяжелые операции по копированию, сворачиванию развертки, и снова копированию(см. Алгоритм поиска всех путей).

Стоит отметить, что такая оптимизация будет полезна не для всех задач по свертыванию. 

Если оси ни разу не накладываются друг на друга так, чтобы их снова можно было свернуть (рис. 12), то такой алгоритм работает даже хуже предыдущего, поскольку после каждого сворачивания перебирает все предыдущие развертки в поисках дубликатов. А также хранит дополнительные данные.

![Рис. 12. Единственный путь решения: DHCGBFAE.](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/Untitled%204.png)

Рис. 12. Единственный путь решения: DHCGBFAE.
...
![            Рисунок 2. Paddings.](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/how_it_should_work.drawio%201.svg)
...
class SolutionTree(Tree):
	...
	**doublesPaths:** lst
	...
	def __**create_tree_alg**(parent='root')
...
![Пример “чистой гармошки”](%D0%94%D0%BE%D0%BA%D0%B0%20fc9416aa3ecf4fa0a46121476e823646/Untitled%205.png)
